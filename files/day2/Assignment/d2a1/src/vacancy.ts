...
export class Vacancy {
    ...
    display1(vac: vacancy[]): any {

        // localStorage.setItem("vacancy", JSON.stringify(vac))
        let table = <HTMLInputElement>document.getElementById('vacancy')
        let tbl: any = "";
    ...
    addVacancy(): any {
        ...
        let cartItems: any = localStorage.getItem("vacancy");
        let vacancystorage = JSON.parse(cartItems) || [];
        console.log(vacancystorage.length+1);
        
        let vaca: vacancy = { vacancy_id: vacancystorage.length+1 , vacancy_name: vname, description: vdescription, Total_Number: totalvacancy }
